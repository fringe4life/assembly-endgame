...
const button = cva({
    ...
    variants: {
        intent: {
            unpressed: 'bg-amber-400 hover:scale-110 cursor-pointer active:translate-y-0 hover:-translate-y-0.5 hover:bg-amber-500 hover:shadow-sm hover:shadow-gray-500 scale-105',
...
const Key: React.FC<ButtonProps> = ({className,  intent,  children}) => {
    ...
    return (
        <button type="button" onClick={handleKeyClick} className={button({ intent, className })}>
            {children}
        </button>
    )
}
